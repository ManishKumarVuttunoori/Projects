##MANISH KUMAR VUTTUNOORI
##email : manivutt@umail.iu.edu
## I have discussed with SravyaGudipudi and Arnab Kumar Mishra on this assignment about the given sample code structure
import time
from heapq import *
import functools
import copy
...
### breadth first search implementation by cosntructing a queue
def breadthFirst(queue,visited):
    global pri
    num,world = heappop(queue)
    ...
    for action in world.allowedActions():
        ...
        if not duplicateWorld(neighbor, visited):
            pri = pri+1
            heappush(queue,(pri,neighbor))
    return queue,world
### breadth first search implementation by cosntructing a stack by giving negative priorities to the worlds that enter the queue( This makes priority queue a stack)
def depthFirst(queue,visited):
    ...
    for action in world.allowedActions():
        ...
        if not duplicateWorld(neighbor, visited):
            ...
            heappush(queue,(pri,neighbor))
    return queue,world
### priority value is calculated by taking the heuristic into consideration, Best First
def bestFirst(queue,visited,heuristic):
    ...
    for action in world.allowedActions():
        ...
        deltaCost = neighbor.do(action)
        if not duplicateWorld(neighbor, visited):
            pri = heuristic(neighbor)
            heappush(queue,(pri,neighbor))
    return queue,world
### priority value is calculated by taking the heuristic as well as the cost of the world into consideration, A star
def aStar(queue,visited,heuristic):
    ...
    for action in world.allowedActions():
        ...
        deltaCost = neighbor.do(action)
        if not duplicateWorld(neighbor, visited):
            pri = heuristic(neighbor) +  neighbor.cost
            heappush(queue,(pri,neighbor))
    return queue,world
...
def heuristic_first(currentworld):      ### computes the distances of the hand from the block and from block to the goal. If in between any of the block reaches the goal... then the
                                                       ### distance of hand from that block will not be considered into the computation 
    ##get the hand's coordinates
    priority_value = 0
    hand_distance = []
    block_distance = []
    (hand0, hand1, hand2) = currentworld.hand['location']
    ## loop through objects in the world and compute the distances from the hand to the blocks and distances from blocks to their goals...... 
    for world_object in currentworld.blocks.keys():
        obj_block_x, obj_block_y, obj_block_z  = currentworld.blocks[world_object]
        obj_goal_x, obj_goal_y, obj_goal_z  = currentworld.goal[world_object]
        block_distance.append(math.sqrt((obj_block_x - obj_goal_x)**2 + (obj_block_y - obj_goal_y)**2 ))
        hand_distance.append(math.sqrt((obj_block_x - hand0)**2 + (obj_block_y - hand1)**2 ))
    for i in range(len(block_distance)):
        if block_distance[i]==0:
            hand_distance[i]=0
    priority_value = sum(block_distance)+sum(hand_distance)        
    return priority_value

def heuristic_second(currentworld):           ### computes the manhattan distances of the hand from the block and from block to the goal. If in between any of the block reaches the goal... then the
                                                       ### manhattan  distance of hand from that block will not be considered into the computation
    ##get the hand's coordinates
    priority_value = 0
    hand_distance = []
    block_distance = []
    ...
    for world_object in currentworld.blocks.keys():
        obj_block_x, obj_block_y, obj_block_z  = currentworld.blocks[world_object]
        obj_goal_x, obj_goal_y, obj_goal_z  = currentworld.goal[world_object]        
        block_distance.append(abs(obj_block_x - obj_goal_x)+abs(obj_block_y - obj_goal_y))
        hand_distance.append(abs(obj_block_x - hand0)+abs(obj_block_y - hand1))
         #priority_value+= abs(obj_block_x - obj_goal_x)+abs(obj_block_y - obj_goal_y)+abs(obj_block_x - hand0)+abs(obj_block_y - hand1)
    for i in range(len(block_distance)):
        if block_distance[i]==0:
            hand_distance[i]=0
    priority_value = sum(block_distance)+sum(hand_distance)
...
hs = {  heuristic_first, heuristic_second } #this is a set, put your heuristic functions in here.
